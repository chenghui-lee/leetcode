class Solution {
    ...
    boolean dfs(char[][] board, int i, int j, String word, boolean[][] visited){
        ...
        visited[i][j] = true;
        boolean next = dfs(board, i - 1, j, word.substring(1), visited) || dfs(board, i, j - 1, word.substring(1), visited) || dfs(board, i, j + 1, word.substring(1), visited) || dfs(board, i + 1, j, word.substring(1), visited);
        visited[i][j] = false;
        return next;
    }
...
// another solution in c++, using trie,  @haruhiku
class Solution {
public:
    struct TrieNode {
